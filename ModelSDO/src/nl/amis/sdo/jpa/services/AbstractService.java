...
public abstract class AbstractService implements Service {
  ...
  public <S extends BaseDataObject<T>, T extends BaseEntity<S>> S get(final Class<T> implementation,
                                                                      final Object id) {
  ...
  @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
  public <S extends BaseDataObject<T>, T extends BaseEntity<S>> S update(final Class<T> implementation,
                                                                         final S dataObject) {
    logger.log(Level.FINEST, "implementation: {0}", implementation);
    logger.log(Level.FINEST, "dataObject: {0}", dataObject);
    if (dataObject != null) {
  ...
  @TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
  public <S extends BaseDataObject<T>, T extends BaseEntity<S>> void delete(final Class<T> implementation,
                                                                            final S dataObject) {
    logger.log(Level.FINEST, "implementation: {0}", implementation);
    logger.log(Level.FINEST, "dataObject: {0}", dataObject);
    if (dataObject != null) {
      getEntityManager().remove(getEntityManager().find(implementation,
                                                        dataObject.toEntity().getId()));
    }
  }

  public <S extends BaseDataObject<T>, T extends BaseEntity<S>> Long count(final Class<T> implementation,
                                                                           final FindCriteria findCriteria,
                                                                           /* JSR-227 API - BC4J Service Runtime */final FindControl findControl) /* BC4J Service Runtime */ {
  ...
  public <S extends BaseDataObject<T>, T extends BaseEntity<S>> List<S> find(final Class<T> implementation,
                                                                             final FindCriteria findCriteria,
                                                                             /* JSR-227 API - BC4J Service Runtime */final FindControl findControl) /* BC4J Service Runtime */ {
    final CriteriaBuilder cb = getEntityManager().getCriteriaBuilder();
  ...
  protected <S extends BaseDataObject<T>, T extends BaseEntity<S>> ProcessData process(final Class<T> implementation,
                                                                                       final ProcessData request) {
  ...
  protected <S extends BaseDataObject<T>, T extends BaseEntity<S>> List<S> process(final Class<T> implementation,
                                                                                   final String operation,
                                                                                   final List<S> dataObjects) {
    if (dataObjects != null) {
      for (int index = 0; index < dataObjects.size(); index++) {
        logger.log(Level.FINEST, "operations[{0}]: {1}",
                   new Object[] { index, operation });

        if ("Create".equals(operation)) {
          dataObjects.set(index, create(dataObjects.get(index)));
        } else if ("Merge".equals(operation) || "Update".equals(operation)) {
          dataObjects.set(index,
                          update(implementation, dataObjects.get(index)));
  ...
  protected <T> void appendToQueryBuilder(final CriteriaBuilder cb,
  ...
  protected <T> List<Order> buildOrderBy(final CriteriaBuilder cb,
                                         final CriteriaQuery cq,
                                         final Root<T> root,
                                         final FindCriteria findCriteria) {
    ...
    if (findCriteria.getSortOrder() != null) {
      final List<SortAttribute> sortAttributes =
        findCriteria.getSortOrder().getSortAttribute();
      ...
      if (sortAttributes != null) {
        ...
        for (final SortAttribute sortAttribute : sortAttributes) {
          ...
          logger.log(Level.FINEST, "descending: {0}",
                     sortAttribute.getDescending());
          logger.log(Level.FINEST, "name: {0}", sortAttribute.getName());

          orders.add(sortAttribute.getDescending() ?
                     cb.desc(root.get(camelize(sortAttribute.getName()))) :
                     cb.asc(root.get(camelize(sortAttribute.getName()))));
        }
  ...
  protected <T> Predicate buildWhere(final CriteriaBuilder cb,
                                     final CriteriaQuery cq,
                                     final Root<T> root,
                                     final FindCriteria findCriteria) {
        ...
        for (final ViewCriteriaRow group : groups) {
          ...
          if (items != null) {
            for (final ViewCriteriaItem item : items) {
              ...
              logger.log(Level.FINEST, "upperCaseCompare: {0}",
                         item.getUpperCaseCompare());
              ...
              if ((item.getValue() != null) && !item.getValue().isEmpty()) {
                logger.log(Level.FINEST, "first value: {0}",
                           item.getValue().get(0));

                if ("=".equals(item.getOperator())) {
                  wherePredicates.add(add(cb,
                                          cb.equal(root.get(camelize(item.getAttribute())),
                                                   item.getValue().get(0)),
                                          item));
                } else if ("<>".equals(item.getOperator())) {
                  wherePredicates.add(add(cb,
                                          cb.notEqual(root.get(camelize(item.getAttribute())),
                                                      item.getValue().get(0)),
                                          item));
                } else if ("like".equals(item.getOperator())) {
                  final String value =
                    new StringBuilder().append(item.getValue().get(0)).append("%").toString();
                  logger.log(Level.FINEST, "string value: {0}", value);
                  final Path<String> path =
                    root.get(camelize(item.getAttribute()));

                  if (item.getUpperCaseCompare()) {
                    wherePredicates.add(add(cb,
                                            cb.like(cb.upper(path), value.toUpperCase()),
                                            item));
                  } else {
                    wherePredicates.add(add(cb,
  ...
  protected Predicate add(final CriteriaBuilder cb, final Predicate predicate,
                          final ViewCriteriaItem item) {
